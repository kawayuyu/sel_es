  ...
  describe 'WOOPの法則を浸かった目標の設定' do
    context '目標設定ができるとき' do
      ...
      it '「結果」部分を入力していれば登録できる' do
        @objective.outcome = '結果'
        expect(@objective).to be_valid
      end
      it '「障害」部分を入力していれば登録できる' do
        @objective.obstacle = '障害'
        expect(@objective).to be_valid
      end
    ...
    context '目標設定ができないとき' do
      ...
      it '「願望」部分が空欄だと登録できない' do
        @objective.wish = nil
        @objective.valid?
        expect(@objective.errors.full_messages).to include('願望を入力してください')
      end
      it '「結果」部分が空欄だと登録できない' do
        @objective.outcome = nil
        @objective.valid?
        expect(@objective.errors.full_messages).to include('結果を入力してください')
      end
      it '「障害」部分が空欄だと登録できない' do
        @objective.obstacle = nil
        @objective.valid?
        expect(@objective.errors.full_messages).to include('障害を入力してください')
      end
      it '「計画」部分が空欄だと登録できない' do
        @objective.plan = nil
        @objective.valid?
        expect(@objective.errors.full_messages).to include('計画を入力してください')
      end
    end
  end
end
